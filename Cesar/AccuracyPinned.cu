...
// int nx: nx elementos
// int ny: ny elementos
__global__ void accuracy_score(float *y_true, float *y_pred, float *accuaracy, int nx, int ny)
{
...
int main()
{
    ...
    accuracy_score<<<grid, block>>>(targetValues, predictions, d_accuracy, nx, ny);
